...
Номер версии берётся только из `app/build.gradle.kts`. Для каждого крупного пользовательского
изменения (новая функция, заметное изменение поведения/совместимости или существенное исправление):

- сравни текущую ветку с `main` или явно указанной базовой release-веткой и добавь краткий пункт в
  верхнюю сноску «Что изменено» в `README.md`; сноска должна оставаться накопительным списком
  изменений текущего релиза, поэтому не удаляй из неё другие актуальные пункты;
- увеличь `versionCode` минимум на 1 и обнови `versionName` в `app/build.gradle.kts`. Если целевая
  версия не задана, повышай patch-компонент `versionName`; если версия уже была повышена в текущем
  наборе изменений, повторно её не увеличивай;
- синхронизируй номер версии в сноске README с `versionName`, а базовую версию в заголовке с веткой,
  относительно которой составлен список.

Чистые рефакторинги, тесты, документация и служебные изменения без изменения пользовательского
поведения сами по себе не требуют повышения версии и нового пункта в сноске. `versionCode` всегда
должен расти монотонно.
...
3. Для крупного пользовательского изменения обнови верхнюю сноску README и версию по правилам
   раздела «Release и baseline profile»; также обнови `docs/` и versioned contract, если применимо.
